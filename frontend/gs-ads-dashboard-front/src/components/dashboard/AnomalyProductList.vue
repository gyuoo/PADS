<template>
  <div class="bg-white rounded-2xl shadow-lg p-6 w-full h-full">
    ...
    >
      <div>이상 상품 현황</div>
      <Badge class="bg-blue-100 text-blue-800 text-lg"
        >총 {{ totalElements }} 건</Badge
      >
    </div>
        ...
        <thead>
          <tr>
            ...
            >
              ...
              <span>
                <img
                  :src="getSortIcon(column.field)"
                  alt="정렬 아이콘"
                  class="inline-block w-4 h-4 ml-3"
                />
              </span>
            </th>
          </tr>
          ...
          >
            ...
            <td class="px-4 py-2 text-sm text-gray-700 whitespace-nowrap">
              <div class="flex space-x-2 items-center">
                <template v-if="product.anomalyCodes.length > 3">
                  <Badge
                    v-for="(anomaly, index) in product.anomalyCodes.slice(0, 3)"
                    :key="index"
                    class="mt-1"
                  >
                  ...
                  <span class="text-red-700"
                    >+{{ product.anomalyCodes.length - 3 }}</span
                  >
                </template>
                <template v-else>
                  <Badge
                    v-for="(anomaly, index) in product.anomalyCodes"
                    :key="index"
                  >
                    {{ getAnomalyDescription(anomaly) }}
                  </Badge>
            ...
            <td class="px-4 py-2 text-sm text-gray-700">
              <div class="flex items-center">
                <span class="w-12 text-sm font-semibold text-gray-700"
                  >{{ product.totalScore }} 점</span
                >
                <div class="w-2/3 bg-gray-200 rounded-full h-3">
                  <div
                    ...
                    :class="{
                      'bg-orange-600':
                        product.totalScore >= 50 && product.totalScore < 70,
                      'bg-red-600':
                        product.totalScore >= 70 && product.totalScore < 90,
...
import { getAnomalyProducts } from '../../api/dashboard.ts'
...
async function fetchAnomalyProducts() {
  try {
    ...
    const response = await getAnomalyProducts(
      '',
      '',
      70,
      currentPage.value - 1,
